 ...
 * @author Erwin_Yoga
 */
@WebServlet(name = "LoadRatingFeedbackReport", urlPatterns = {"/LoadRatingFeedbackReport"})
public class LoadRatingFeedbackReport extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            Double averageScore = ordersFacade.calculateAverageScore(stallName);
            List<Orders> orders = ordersFacade.getOrdersWithCondition(stallName);

            request.setAttribute("averageScore", averageScore);
            request.setAttribute("orders", orders);

            request.getRequestDispatcher("ratingfeedbackreport.jsp").forward(request, response);             
        }
    }
